...
const client = new OAuth2Client(
  "830063440629-j40l5f7lb1fck6ap120s272d49rp1ph6.apps.googleusercontent.com"
);
const clientAi = "";
//  new openai.OpenAI({
//   apiKey: apiKey,
...
const getCandidateInfo = async (req, res) => {
  try {
    ...
    const specificTest = user.tests.find(
      (test) => test?.testId?.toString() === examId
    );
...
const changeStatusCandidate = async (req, res) => {
  try {
    ...
    const candidate = exam.candidates.find(
      (c) => c.candidate._id.toString() === userId
    );
    ...
    return res.status(500).send({
      message:
        "Si è verificato un errore durante la modifica dello status del candidato",
      data: error.message,
      success: false,
    });
  }
...
const addCandidate = async (req, res) => {
  try {
    ...
    if (!candidate) {
      ...
      const existingTest = candidate.tests.find(
      ...
      const specificTest = updatedCandidate.tests.find(
        (test) => test?.testId?.toString() === testId
      );
      const token = jwt.sign(
        {
...
const modifyUserData = async (req, res) => {
  ...
  try {
    ...
    let user = await User.findByIdAndUpdate(userId, req.body, {new:true}).select(['-password']);
    ...
    return res.status(500).json({
      message: "Errore durante l'aggiornamento dei dati utente",
      success: false,
    });
  }
};
