...
const createTeam = async (req, res) => {
  try {
    const { fName, lName, jobTitle, email, password, role, company } = req.body;
...
const updateTeamMember = async (req, res) => {
  ...
  if (!isValidOperation) {
    return res.status(400).json({
      message:
        "Invalid updates! Only name, surname, email, password, and role can be updated.",
      success: false,
    });
  }
  ...
  try {
    ...
    Object.keys(updates).forEach(
      (update) => (member[update] = updates[update])
    );
    await member.save();
    return res.status(200).json({
      message: "Dati utente aggiornati con successo",
