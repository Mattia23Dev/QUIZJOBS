...
useEffect(() => {
  if (user && user.tests && user.tests.length > 0 ) {
    const testMatch = user.tests.find(test => test.testId === examData?._id);
    console.log(testMatch)
    if (testMatch){
      const currentTest = testMatch;
  ...
  return (
   examData && (
    ...
    {view==="thanks"&&<div className='thanks-exam-container flex flex-col gap-2'> 
       ...
       <button style={{zIndex: 10}} className='primary-contained-btn' onClick={() => navigate('/user/home')}>Area riservata</button>
    </div>}
    </div>
    :
