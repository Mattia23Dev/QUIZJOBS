...
const CalendarComponent = ({ tour, openTour, setOpenTour }) => {
  ...
  const steps = [
    ...
    {
      content:
        "Vedi a primo impatto il numero di appuntamenti mensilmente, clicca sul giorno per aprire il popup",
      ...
      <Popover placement="top" title={rightDate} content={content}>
        {
          eventsForDate.length > 0 && (
            ...
            // <ul style={{ padding: "20px 0" }}>
            // {
            <div className={eventsForDate.length > 0 ? "event-badge" : ""}>
              {isMobile()
                ? // <li className="li-calendar">
                  eventsForDate.length > 0
                  ? eventsForDate.length
  ...
  return (
    <div className="home-content">
      <div className="top-calendar">
        {isMobile() && (
          <button
            ...
            onClick={() => handleDateClick(eventAppointment)}
          >
            + Aggiungi
          </button>
        )}
        ...
        <div className="filter-appointment elemento1">
          <div>
            <button
              className={
                eventAppointment.isSame(moment().subtract(1, "day"), "day")
                  ? "active"
                  : ""
              }
              onClick={() => setEventAppointment(moment().subtract(1, "day"))}
            >
              Ieri
            </button>
            <button
              className={
                eventAppointment.isSame(moment(), "day") ? "active" : ""
              }
              onClick={() => setEventAppointment(moment())}
            >
              Oggi
            </button>
            <button
              className={
                eventAppointment.isSame(moment().add(1, "day"), "day")
                  ? "active"
                  : ""
              }
              onClick={() => setEventAppointment(moment().add(1, "day"))}
            >
              Domani
            </button>
          </div>
          <DatePicker
            ...
            locale={locale}
            value={eventAppointment}
            clearIcon={false}
            onChange={(date, dateString) => {
              if (dateString) {
                setEventAppointment(moment(dateString));
              }
            }}
          />
        </div>
      ...
      {isMobile() && (
        <div className="help-top" style={{ marginTop: "1rem" }}>
          <div
            ...
            className={mobileView === "app" ? "active" : "elemento2"}
          >
            <span></span>
            <p>Appuntamenti</p>
          </div>
          <hr />
          <div
            onClick={() => setMobileView("cal")}
            className={mobileView === "cal" ? "active" : "elemento3"}
          >
            <span></span>
            <p>Calendario</p>
          </div>
                ...
                <div className="">
                  ...
                  <ul className={mobileView === "app" ? "elemento2" : ""}>
                    ...
                    events?.filter((event) => {
                      ...
                      return (
                        eventDate ===
                        moment(eventAppointment).format("DD-MM-YYYY")
                      );
                    }).length > 0 ? (
                      events
                        ?.filter((event) => {
                          const eventDate = moment(event.date).format(
                            "DD-MM-YYYY"
                          );
                          return (
                            eventDate ===
                        ...
                        .map((event, index) => {
                          const formattedDate = moment(event.date).format(
                            "DD-MM-YYYY"
                          );
                          const formattedTime = moment(event.date).format(
                            "HH:mm"
                          );
                          return (
                            <li
                            ...
                            >
                              ...
                              {event.candidate.name +
                                " " +
                                event.candidate.surname}
                              , <br />
                              <strong>Posizione lavorativa:</strong>{" "}
                              {event.jobPosition}
                            </li>
                          );
                        })
                    ) : (
                      <p style={{ margin: "30px 0", fontWeight: "600" }}>
                        Non ci sono appuntamenti oggi
                      </p>
                    )}
                  </ul>
                </div>
      ...
      >
        ...
        <p>
          {moment(drawerEvent?.date).format("DD-MM-YYYY")} -{" "}
          {moment(drawerEvent?.date).format("HH:mm")}
        </p>
        ...
        <div className="modal-candidate-data">
          <div>
            <p>Nome:</p>
            <p>
              {drawerEvent?.candidate.name +
                " " +
                drawerEvent?.candidate.surname}
            </p>
          </div>
          <div>
            <p>Email:</p>
          ...
          <a
            className="allegati"
            href={`https://quizjobs-production.up.railway.app/uploads/${drawerEvent?.candidate.cv}`}
          >
            <img src={allegati} alt="documento link" />
            Scarica CV
          </a>
        </div>
      </Drawer>
