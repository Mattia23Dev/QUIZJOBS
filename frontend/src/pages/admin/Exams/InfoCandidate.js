    ...
    <div className="eduwork">
      <Steps direction="vertical">
        {works?.map((work, index) => (
          <Step
            status="finish"
            icon={<img src={workImg} alt="work skilltest" />}
            key={index}
            description={work}
          />
        ))}
  ...
  return (
    <div className="eduwork">
      <Steps direction="vertical">
        {educ?.map((edu, index) => (
          <Step
            icon={<img src={eduImg} alt="education skilltest" />}
            status="process"
            key={index}
            description={edu}
          />
...
function InfoCandidate(props) {
  ...
  const arraySkills = (skills) => {
    ...
    const skillsArray = cleanedSkillsString
      ?.split("\n")
        ...
        {activeTab === 1 ? (
          <>
            <div className="modal-candidate-middle">
              {tag === "manual" ? (
                ...
                <div className="modal-punteggio">
                  ...
                  <p>
                    <font color="#34C15B">
                      {candidate?.tests[0].correctAnswers}
                    </font>
                    /{candidate?.tests[0].totalQuestions}
                  </p>
                </div>
              )}
              <div className="modal-bar">
                ...
                <Progress
                  percent={candidate?.tests[0]?.report?.result.percentage.toFixed(
                    2
                  )}
                  strokeColor={
                    candidate?.tests[0]?.report?.result.percentage > 60
                      ? "#34C15B"
                      : "#F95959"
                  }
                />
            ...
            <div className="note">
              ...
              />
              {notes !== note && (
                <span onClick={() => updateNotes(selectedCandidate, note)}>
                  Salva
                </span>
              )}
            </div>
            ...
            <a
              ...
              href={
                selectedCandidate.cvUrl
                  ? selectedCandidate.cvUrl
                  : `https://quizjobs-production.up.railway.app/uploads/${selectedCandidate.cv}`
              }
              target="__blank"
            >
            ...
            <a className="add-allegato" style={{ cursor: "not-allowed" }}>
              <span>+</span> Aggiungi allegato
            </a>
          ...
          tag === "manual" ||
          (tag === "mix" && domandaType === "Personalizzate") ? (
            <div className="modal-candidate-domande">
              ...
              {tag !== "mix"
                ? candidate?.tests[0].arrayAnswers.questions.map(
                    (question, index) => {
                      const answer =
                        candidate?.tests[0].arrayAnswers.answers[index];
                      console.log(answer);
                      const seconds =
                        candidate?.tests[0].arrayAnswers.seconds[index];
                      const correctAnswer = examQuestion.find(
                      ...
                      return (
                        <div className="modal-manual-question">
                          <div
                            ...
                            className="modal-candidate-question-m"
                            key={index}
                          >
                            ...
                            <div style={{ textAlign: "right" }}>
                              <img alt="secondi domanda" src={time} />
                              <span>
                                00:
                                {seconds
                                  ? seconds < 10
                ...
                : candidate?.tests[0].arrayAnswersPersonal.questions.map(
                    (question, index) => {
                      const answer =
                        candidate?.tests[0].arrayAnswersPersonal.answers[index];
            ...
            <div className="modal-candidate-domande">
              ...
              {candidate?.tests[0]?.arrayAnswers?.questions?.map(
                (question, index) => {
                  ...
                  const isCorrect =
                    correctAnswer && correctAnswer.includes(answer);
          ...
          <div className="cv-extract">
            ...
            <div>
              ...
              <div>
                <WorkExperienceChart
                  works={arraySkills(pdfExtract?.workExperience)}
                />
              </div>
            </div>
            <div>
